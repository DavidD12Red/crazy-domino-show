...
namespace VisualDebugger
{
	...
	// Simulation objects
	Camera* camera;
	PhysicsEngine::DominoScene* scene;
	PxReal delta_time = 1.f/60.f;
	PxReal gForceStrength = 15;
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		scene = new PhysicsEngine::DominoScene();
		scene->SceneInit();
		...
		camera = new Camera(PxVec3(0.f, 5.f, 15.f), PxVec3(0.f,-.1f,-1.f), 5.f);
	...
	void HUDInit()
	{
		...
		hud.AddLine(HELP, "");
		hud.AddLine(HELP, " Swing wrecking balls");
		hud.AddLine(HELP, "    F1");
		// Add a pause screen
		hud.AddLine(PAUSE, "");
		hud.AddLine(PAUSE, "");
	...
	void ForceInput(int key)
	{
		...
		case 'I': //forward
			scene->ball->addForce(PxVec3(0,0,-1)*gForceStrength);
			break;
		case 'K': //backward
			scene->ball->addForce(PxVec3(0,0,1)*gForceStrength);
			break;
		case 'J': //left
			scene->ball->addForce(PxVec3(-1,0,0)*gForceStrength);
			break;
		case 'L': //right
			scene->ball->addForce(PxVec3(1,0,0)*gForceStrength);
			break;
	...
	void KeySpecial(int key, int x, int y)
	{
		...
		{
		case GLUT_KEY_F1:
			for (unsigned i = 0; i < scene->wreckingBalls.size(); i++)
				scene->wreckingBalls[i]->isStart = true;
			break;
			//display control
		case GLUT_KEY_F5:
			//hud on/off
		...
		case GLUT_KEY_F11:
			...
			//scene->Reset();
			scene->ball->Reset();
			for (unsigned i = 0; i < scene->dominoPieces.size(); i++)
				scene->dominoPieces[i]->Reset();
			for (unsigned i = 0; i < scene->wreckingBalls.size(); i++)
				scene->wreckingBalls[i]->Reset();
			break;
		default:
			break;
		}

...
namespace PhysicsEngine
{
	...
	void Actor::SetMass(PxReal new_mass)
	{
		mass = new_mass;
		actor->is<PxRigidBody>()->setMass(mass);
	}

	PxReal Actor::GetMass()
	{
		return mass;
	}
	...
	{
		actor->is<PxRigidBody>()->setGlobalPose(PxTransform(position, actor->is<PxRigidBody>()->getGlobalPose().q));
	}

	PxVec3 Actor::GetPosition()
	{
		if (actor->is<PxRigidBody>())
			return actor->is<PxRigidBody>()->getGlobalPose().p;
		else
			return actor->is<PxRigidStatic>()->getGlobalPose().p;
	}

	void Actor::SetRotation(PxQuat rotation)
	{
		actor->is<PxRigidBody>()->setGlobalPose(PxTransform(actor->is<PxRigidBody>()->getGlobalPose().p, rotation));
	}

	PxQuat Actor::GetRotation()
	{
		if (actor->is<PxRigidBody>())
			return actor->is<PxRigidBody>()->getGlobalPose().q;
		else
			return actor->is<PxRigidStatic>()->getGlobalPose().q;
	}

	PxRigidBody* Actor::GetRigidBody()
	{
		return actor->is<PxRigidBody>();
	}

	PxVec3 Actor::GetVelocity()
	{
		return actor->is<PxRigidBody>()->getLinearVelocity();
	}
	...
	void Scene::SceneUpdate(PxReal dt)
	{
		if (pause)
			return;
